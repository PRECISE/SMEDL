//  Pub/Sub node
//  Compile (OS X): gcc producer.c -L/usr/local/lib -lrabbitmq -I/usr/local/include -o bin/producer
...
int main(int argc, char const *const *argv)
{
  ...
  if (argc < 5) {
    fprintf(stderr, "Usage: producer host port username password bindingkey\n");
    return 1;
  }
  ...
  amqp_exchange_declare(conn, 1, amqp_cstring_bytes("smedl.topic"), amqp_cstring_bytes("topic"),
                        0, 0, 0, 0, amqp_empty_table);
  die_on_amqp_error(amqp_get_rpc_reply(conn), "Declaring exchange");

  send_batch(conn, bindingkey, 10, 100);
