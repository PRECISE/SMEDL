...
# Inheriting from FailedSemantics changes the semantics of parsing. Bad.
...
class TypeMismatch(SmedlException):
    """Raised when the type of an expression does not match what is
    expected."""


class NameNotDefined(SmedlException):
    """Raised when an identifier is used (e.g. event name, state variable,
    etc.) without being defined."""
