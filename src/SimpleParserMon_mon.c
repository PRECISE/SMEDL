...
typedef enum { SIMPLEPARSERMON_POINT_COUNTS_SCENARIO } simpleparsermon_scenario;
typedef enum { SIMPLEPARSERMON_POINT_COUNTS_ERROR, SIMPLEPARSERMON_POINT_COUNTS_READYCOUNTS, SIMPLEPARSERMON_POINT_COUNTS_GEN0, SIMPLEPARSERMON_POINT_COUNTS_GEN1, SIMPLEPARSERMON_POINT_COUNTS_GEN2 } simpleparsermon_point_counts_state;
typedef enum { SIMPLEPARSERMON_GETTIME_EVENT, SIMPLEPARSERMON_GETLAT_EVENT, SIMPLEPARSERMON_GETLON_EVENT, SIMPLEPARSERMON_GETDIST_EVENT } simpleparsermon_event;
typedef enum { SIMPLEPARSERMON_DEFAULT } simpleparsermon_error;
const char *simpleparsermon_point_counts_states[5] = {"Error", "ReadyCounts", "Gen0", "Gen1", "Gen2"};
const char **simpleparsermon_states_names[1] = { simpleparsermon_point_counts_states };
...
SimpleparsermonMonitor* init_simpleparsermon_monitor( SimpleparsermonData *d ) {
    ...
    monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO] = SIMPLEPARSERMON_POINT_COUNTS_READYCOUNTS;
...
void simpleparsermon_getTime(SimpleparsermonMonitor* monitor, int time) {
  switch (monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO]) {
    case SIMPLEPARSERMON_POINT_COUNTS_READYCOUNTS:
...
void simpleparsermon_getLon(SimpleparsermonMonitor* monitor, float lon) {
  switch (monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO]) {
    case SIMPLEPARSERMON_POINT_COUNTS_GEN1:
      if(lon >= -180 && lon <= 180) {
        monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO] = SIMPLEPARSERMON_POINT_COUNTS_GEN2;
      }
      else {
        monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO] = SIMPLEPARSERMON_POINT_COUNTS_ERROR;
      }
...
void simpleparsermon_getDist(SimpleparsermonMonitor* monitor, float dist) {
  switch (monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO]) {
    case SIMPLEPARSERMON_POINT_COUNTS_GEN2:
      if(dist >= 0) {
        monitor->state[SIMPLEPARSERMON_POINT_COUNTS_SCENARIO] = SIMPLEPARSERMON_POINT_COUNTS_READYCOUNTS;
      }
      else {
