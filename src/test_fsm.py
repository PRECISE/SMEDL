...
class TestFSM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s1 = State("s1")
        self.s2 = State("s2")
        self.t1 = Transition(self.s1, self.s2, "SomeEvent", "SomeCondition")
        self.t2 = Transition(self.s2, self.s1, "SomeOtherEvent", "SomeOtherCondition")
    ...
    def test_deleteState(self):
        ...
        # self.fsm.deleteState(self.s2) KeyError if not found
    ...
    def test_getTransitionByAction(self):
        self.assertRaises(TypeError, self.fsm.getTransitionsByAction, 1)        
        self.assertRaises(TypeError, self.fsm.getTransitionsByAction, self.t1) 
        self.fsm.addState(self.s1) 
        self.fsm.addState(self.s2)
        self.fsm.addTransition(self.t1) 
        self.fsm.addTransition(self.t2)
        self.assertEqual(2, len(self.fsm.transitions))
        self.assertFalse(self.fsm.getTransitionsByAction("nothing")) 
        self.assertEqual(1, len(self.fsm.getTransitionsByAction("SomeEvent")))
        t3 = Transition(self.s2, self.s1, "SomeEvent", "SomeOtherCondition") 
        self.fsm.addTransition(t3)
        self.assertEqual(3, len(self.fsm.transitions))
        self.assertEqual(2, len(self.fsm.getTransitionsByAction("SomeEvent")))             
