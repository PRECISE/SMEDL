//Compile: gcc -o SimpleParserMon2_mon -std=c99 actions.c monitor_map.c SimpleParserMon2_mon.c

#include <stdio.h>
#include <stdlib.h>
#include "SimpleParserMon2_mon.h"
...
typedef enum { SIMPLEPARSERMON_GETLAT, SIMPLEPARSERMON_GETLON, SIMPLEPARSERMON_GETTIME, SIMPLEPARSERMON_GETSPEED, SIMPLEPARSERMON_GETDIST } simpleparsermon_event;
...
void simpleparsermon_getLat(SimpleparsermonMonitor* monitor, int lat) {
...
void raise_simpleparsermon_getLat(SimpleparsermonMonitor* monitor, int lat) {
  param *p_head = NULL;
  push_param(&p_head, &lat, NULL, NULL, NULL);
  push_action(&monitor->action_queue, SIMPLEPARSERMON_GETLAT, p_head);
}


void simpleparsermon_getLon(SimpleparsermonMonitor* monitor, int lon) {
  switch (monitor->state[SIMPLEPARSERMON_POINT_COUNTS]) {
...
void raise_simpleparsermon_getLon(SimpleparsermonMonitor* monitor, int lon) {
  param *p_head = NULL;
  push_param(&p_head, &lon, NULL, NULL, NULL);
...
void simpleparsermon_getSpeed(SimpleparsermonMonitor* monitor, int speed) {
  switch (monitor->state[SIMPLEPARSERMON_POINT_COUNTS]) {
    default:
...
void raise_simpleparsermon_getSpeed(SimpleparsermonMonitor* monitor, int speed) {
  param *p_head = NULL;
  push_param(&p_head, &speed, NULL, NULL, NULL);
  push_action(&monitor->action_queue, SIMPLEPARSERMON_GETSPEED, p_head);
}


void simpleparsermon_getDist(SimpleparsermonMonitor* monitor, int dist) {
  switch (monitor->state[SIMPLEPARSERMON_POINT_COUNTS]) {
    default:
...
void raise_simpleparsermon_getDist(SimpleparsermonMonitor* monitor, int dist) {
  param *p_head = NULL;
  push_param(&p_head, &dist, NULL, NULL, NULL);
