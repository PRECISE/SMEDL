  ...
  switch (monitor->state[RATECOMPUTATION_COMPUTATION_SCENARIO]) {
    ...
    case RATECOMPUTATION_COMPUTATION_COMPUTERATE:
      if(monitor->curTime - monitor->lastTime > 0) {
        ...
        raise_ratecomputation_dataUpdate2(monitor, monitor->name, monitor->curTime, monitor->rate);
        monitor->sum = 0;
...
void raise_ratecomputation_dataUpdate2(RatecomputationMonitor* monitor, char* v0, double v1, double v2) {
 ...
 param *ep_head = NULL;
  ...
  push_param(&p_head, NULL, NULL, &v1, NULL);
  push_param(&ep_head, NULL, NULL, &v1, NULL);
  push_param(&p_head, NULL, NULL, &v2, NULL);
  push_param(&ep_head, NULL, NULL, &v2, NULL);
