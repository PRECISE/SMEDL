//Compile: gcc -o string_mon -std=c99 actions.c monitor_map.c string_mon.c
...
typedef enum { STRINGTEST_SC1_SCENARIO } stringtest_scenario;
typedef enum { STRINGTEST_SC1_START, STRINGTEST_SC1_END } stringtest_sc1_state;
typedef enum { STRINGTEST_PING_EVENT, STRINGTEST_PONG_EVENT } stringtest_event;
typedef enum { STRINGTEST_DEFAULT } stringtest_error;
const char *stringtest_sc1_states[2] = { "Start", "End" };
const char **stringtest_states_names[1] = { stringtest_sc1_states };
...
StringtestMonitor* init_stringtest_monitor( StringtestData *d ) {
    ...
    config_init(&cfg);
    if(!config_read_file(&cfg, "string_mon.cfg")) {
...
st= cJSON_GetObjectItem(fmt,"v1")->valuestring;
	x= cJSON_GetObjectItem(fmt,"v2")->valueint;
                        ...
                        stringtest_ping(monitor, st, x, pro);
...
pro = ((params)->provenance);
		...
		pop_param(&p_head);
		pop_action(head);
		stringtest_pong(monitor, v0, i0, pro);
...
pro = ((params)->provenance);
		...
		pop_param(&p_head);
		pop_action(head);
		exported_stringtest_pong(monitor, v0, i0, pro);
...
if (executed_scenarios[STRINGTEST_SC1_SCENARIO]==0) {
  switch (monitor->state[STRINGTEST_SC1_SCENARIO]) {
    case STRINGTEST_SC1_START:
        raise_stringtest_pong(monitor, st, x,provenance);
...
void stringtest_pong(StringtestMonitor* monitor, char* st, int x, cJSON * provenance) {
executeEvents(monitor);
}
