...
class TestSpvRabbitMQ(unittest.TestCase):
    ...
    def test_spvrabbitmq_run(self):
        ...
        example = ExampleConsumer('amqp://'+c.rabbitmq.username+':'+c.rabbitmq.password+'@'+c.rabbitmq.hostname+':'+str(c.rabbitmq.port)+'/%2F')
        example.EXCHANGE = c.rabbitmq.ctrl_exchange
        example.EXCHANGE_TYPE = 'fanout'
        example.run()
